...
var Typo = /** @class */ (function() {
   ...
   *
   */
  function Typo(affData, wordsData) {
  ...
  Typo.prototype.suggest = function(word, limit) {
    ...
    function edits1(words, knownOnly) {
      ...
      for (var _i = 0, words_1 = words; _i < words_1.length; _i++) {
        var word_2 = words_1[_i];
        for (i = 0, len = word_2.length + 1; i < len; i++) {
          var s = [word_2.substring(0, i), word_2.substring(i)];
          if (s[1]) {
  ...
  Typo.prototype.setup = function() {
    ...
    this.compoundRules.forEach(function(rule) {
      rule.forEach(function(item) {
        return (_this.compoundRuleCodes[item] = []);
      });
    });
    ...
    this.compoundRules.map(function(ruleText) {
      var expressionText = "";
      ruleText.forEach(function(character) {
        expressionText +=
          character in _this.compoundRuleCodes
            ? "(" + _this.compoundRuleCodes[character].join("|") + ")"
            : character;
      });
   ...
   */
  Typo.prototype.removeAffixComments = function(data) {
    // Remove comments
                ...
                for (var k = j + 1; k < jlen; k++) {
                  ...
                  if (combineRule) {
                    if (
                      ...
                      for (
                        var iii = 0, iiilen = otherNewWords.length;
                        iii < iiilen;
                        iii++
                      ) {
   ...
   * @return {String} The cleaned-up data.
   */
  Typo.prototype.removeDicComments = function(data) {
  ...
  Typo.prototype.parseRuleCodes = function(textCodes) {
    if (textCodes == null) {
      ...
      var flags = [];
      for (var i = 0; i < textCodes.length; i += 2) {
    ...
    for (var i = 0, len = entries.length; i < len; i++) {
      ...
      if (!entry.match || word.match(entry.match)) {
        ...
        if ("continuationClasses" in entry) {
          for (
            var j = 0, jlen = entry.continuationClasses.length;
            j < jlen;
            j++
          ) {
