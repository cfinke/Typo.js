...
var Typo = /** @class */ (function() {
  ...
  function Typo(dictionary, affData, wordsData, settings) {
    ...
    this.memoized = {};
    this.loaded = false;
    var path = Typo.DEFAULT_PATH;
    if (dictionary) {
      // If the data is preloaded, just setup the Typo object.
      if (this.affData && this.wordsData) {
        this.setup();
      }
      if (this.settings.dictionaryPath) {
        path = this.settings.dictionaryPath;
      }
      if (!this.affData) {
        this.readDataFile(
  ...
  Typo.prototype.readDataFile = function(url, setFunc) {
  ...
  Typo.prototype.setAffData = function(data) {
    this.affData = data;
    if (this.wordsData) {
      this.setup();
    }
  };
  Typo.prototype.setWordsData = function(data) {
    this.wordsData = data;
    if (this.affData) {
      this.setup();
    }
  ...
  Typo.prototype.setup = function() {
    ...
    this.compoundRuleCodes = {};
    var _len = this.compoundRules.length;
    for (var i = 0; i < _len; i++) {
      var rule = this.compoundRules[i];
      var _jlen = rule.length;
      for (var j = 0; j < _jlen; j++) {
    ...
    this.dictionaryTable = this._parseDIC(this.wordsData);
    ...
    for (var i = 0, _len_1 = this.compoundRules.length; i < _len_1; i++) {
      var ruleText = this.compoundRules[i];
      var expressionText = "";
      var _jlen = ruleText.length;
      for (var j = 0; j < _jlen; j++) {
   ...
   *          always returned.
   */
  Typo.prototype.readFile = function(path, charset, asyncLoad) {
    charset = charset || "utf8";
    if (typeof XMLHttpRequest !== "undefined") {
      var promise;
      var req = new XMLHttpRequest();
      req.open("GET", path, asyncLoad);
      if (asyncLoad) {
      ...
      req.send(null);
      return asyncLoad ? promise : req.responseText;
    } else {
      try {
   ...
   */
  Typo.prototype._parseAFF = function(data) {
    var rules = {};
   ...
   */
  Typo.prototype._removeAffixComments = function(data) {
    // Remove comments
   ...
   */
  Typo.prototype._parseDIC = function(data) {
   ...
   * @return {String} The cleaned-up data.
   */
  Typo.prototype._removeDicComments = function(data) {
  ...
  Typo.prototype.parseRuleCodes = function(textCodes) {
    if (!textCodes) {
      return [];
   ...
   */
  Typo.prototype._applyRule = function(word, rule) {
   ...
   * @returns {Boolean}
   */
  Typo.prototype.check = function(aWord) {
    if (!this.loaded) {
   ...
   * @returns {Boolean}
   */
  Typo.prototype.checkExact = function(word) {
    if (!this.loaded) {
      throw "Dictionary not loaded.";
    }
   ...
   * @return {Boolean}
   */
  Typo.prototype.hasFlag = function(word, flag, wordFlags) {
    if (!this.loaded) {
      throw "Dictionary not loaded.";
    }
  ...
  Typo.prototype.suggest = function(word, limit) {
    ...
    function edits1(words, known_only) {
      ...
      for (var word in words) {
        for (i = 0, _len = word.length + 1; i < _len; i++) {
          ...
          if (s[1]) {
            for (j = 0, _jlen = this.alphabet.length; j < _jlen; j++) {
              // Eliminate replacement of a letter by itthis
              if (this.alphabet[j] != s[1].substring(0, 1)) {
                _edit = s[0] + this.alphabet[j] + s[1].substring(1);
                if (!known_only || this.check(_edit)) {
                  if (!(_edit in rv)) {
                    rv[_edit] = 1;
                  } else {
    ...
    function correct(word) {
      ...
      for (var i_1 in weighted_corrections) {
        if (weighted_corrections.hasOwnProperty(i_1)) {
          sorted_corrections.push([i_1, weighted_corrections[i_1]]);
        }
      }
      function sorter(a, b) {
        if (a[1] < b[1]) {
      ...
      for (i = 0; i < Math.min(working_limit, sorted_corrections.length); i++) {
        ...
        if (
          !this.hasFlag(sorted_corrections[i][0], "NOSUGGEST") &&
