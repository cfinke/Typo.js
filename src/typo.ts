...
export class Typo {
    ...
    private dictionaryTable: Object = {};

    private compoundRules: Array<any> = [];
    private compoundRuleCodes: { [rule: string]: Array<any> } = {};
    private replacementTable: Array<any> = [];
    ...
    private readDataFile(url: string, setFunc: ((data: string) => void)) {
        const response = this.readFile(url, null, this.settings.asyncLoad);
    ...
    private setAffData(data: string) {
        this.affData = data;
    ...
    private setWordsData(data: string) {
        this.wordsData = data;
    ...
    private setup(): void {
        ...
        const _len = this.compoundRules.length;
        for (let i = 0; i < _len; i++) {
            let rule = this.compoundRules[i];

            const _jlen = rule.length;
            for (let j = 0; j < _jlen; j++) {
        ...
        for (let i in this.compoundRuleCodes) {
    ...
    _removeAffixComments(data: string) {
        // Remove comments
            ...
            if (!entry.match || word.match(entry.match)) {
                ...
                if ("continuationClasses" in entry) {
                    for (
                        ...
                        var continuationRule = this.rules[
    ...
    suggest(word: string, limit: number): Array<string> {
        if (!this.loaded) {
            throw "Dictionary not loaded.";
        }
        ...
        function edits1(words, known_only?) {
            ...
            for (var word in words) {
                for (i = 0, _len = word.length + 1; i < _len; i++) {
                    ...
                    if (s[1]) {
                        for (
                            j = 0, _jlen = this.alphabet.length;
                            j < _jlen;
                            j++
                        ) {
        ...
        function correct(word) {
            ...
            for (let i in weighted_corrections) {
                if (weighted_corrections.hasOwnProperty(i)) {
                    sorted_corrections.push([i, weighted_corrections[i]]);
                }
            }

            function sorter(a: Array<number>, b: Array<number>) {
                if (a[1] < b[1]) {
                    return -1;
                }
            ...
            let working_limit: number = limit;
            ...
            for (
                ...
                if ("uppercase" === capitalization_scheme) {
                    sorted_corrections[i][0] = sorted_corrections[
                        i
                    ][0].toUpperCase();
