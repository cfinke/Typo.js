...
export class Typo {
    ...
    private dictionaryTable: { [word: string]: any } = {};

    private compoundRules: any[] = [];
    private compoundRuleCodes: { [rule: string]: any[] } = {};
    ...
    constructor(
        ...
        private setup(): void {
            ...
            let len = this.compoundRules.length;
            for (let i = 0; i < len; i++) {
                const rule = this.compoundRules[i];

                const jlen = rule.length;
            ...
            len = this.compoundRules.length;
            for (let i = 0; i < len; i++) {
                const ruleText = this.compoundRules[i];

                let expressionText = "";

                const jlen = ruleText.length;
                for (let j = 0; j < jlen; j++) {
         ...
         * @returns {String} The file data if async is false, otherwise a promise object.
         *          If running node.js, the data is
         *          always returned.
         */
        ...
        private parseRuleCodes(textCodes) {
            if (!textCodes) {
                ...
                for (let i = 0, len = textCodes.length; i < len; i += 2) {
        ...
        private suggest(word: string, limit: number): string[] {
            if (!this.loaded) {
                throw new Error("Dictionary not loaded.");
            }
