...
(() => {
    ...
    Typo = function(dictionary, affData, wordsData, settings) {
        ...
        function setup() {
            ...
            for (i = 0, len = self.compoundRules.length; i < len; i++) {
                const rule = self.compoundRules[i];

                for (j = 0, jlen = rule.length; j < jlen; j++) {
            ...
            for (i = 0, len = self.compoundRules.length; i < len; i++) {
                ...
                for (j = 0, jlen = ruleText.length; j < jlen; j++) {
    ...
    Typo.prototype = {
         ...
         *
         * @param object obj A hash of Typo properties,
         * probably gotten from a JSON.parse(JSON.stringify(typo_instance)).
         */
                                    ...
                                    for (let k = j + 1; k < jlen; k++) {
                                        ...
                                        if (combineRule) {
                                            if (combineRule.combineable && (rule.type !== combineRule.type)) {
                                                ...
                                                for (let iii = 0, iiilen = otherNewWords.length; iii < iiilen; iii++) {
            ...
            for (let i = 0, len = entries.length; i < len; i++) {
                ...
                if (!entry.match || word.match(entry.match)) {
                    ...
                    if ("continuationClasses" in entry) {
                        for (let j = 0, jlen = entry.continuationClasses.length; j < jlen; j++) {
        ...
        suggest(word, limit) {
            ...
            /**
             * Returns a hash keyed by all of the strings that
             * can be made by making a single edit to the word (or words in) `words`
                ...
                for (const word in words) {
                    if (words.hasOwnProperty(word)) {
                        for (i = 0, len = word.length + 1; i < len; i++) {
                            ...
                            if (s[1]) {
                                for (j = 0, jlen = self.alphabet.length; j < jlen; j++) {
                                    // Eliminate replacement of a letter by itself
                                    if (self.alphabet[j] !== s[1].substring(0, 1)) {
                                        edit = s[0] + self.alphabet[j] + s[1].substring(1);
            ...
            function correct(word) {
                ...
                for (i = 0; i < Math.min(workingLimit, sortedCorrections.length); i++) {
                    ...
                    if (
                        !self.hasFlag(sortedCorrections[i][0], "NOSUGGEST")
